...
export function* updatingStockAvailable() {
  ...
  try {
    ...
    response.result? itemsFetchingFromStock() : yield put(stockError(response.message));
  } 
  catch (err){
...
export function* itemRemovingFromStock() {
  ...
  try {
    ...
    response.result ? yield put(stockItemRemoved(itemCode)) : yield put(stockError(response.message))
  } 
  catch (err){
    yield put(stockError(err));
  }
